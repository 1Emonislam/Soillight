        ...
        if (shop) {
            if (req?.user?.isAdmin === true) {
                ...
                if (productCreated) {
                    ...
                    const resData = await Product.findOne({ _id: productCreated?._id }).populate("category", "_id category img age").populate("subCategory", "_id subCategory img age").populate("insideSubCategory", "_id insideSubCategory").populate("packType", "_id packType").populate("servingSize","_id servingSize").populate("shop","_id name address location phone email closeDate openDate status");
...
const productUpdate = async (req, res, next) => {
    ...
    try {
        if (!(req?.user?.role === 'seller' || req?.user?.isAdmin === true)) {
            ...
            }, { new: true }).populate("category", "_id category img age").populate("subCategory", "_id subCategory img age").populate("insideSubCategory", "_id insideSubCategory").populate("packType", "_id packType").populate("servingSize","_id servingSize").populate("shop","_id name address location phone email closeDate openDate status");
            if (!productUpdated) {
...
const getSignleProduct = async (req, res, next) => {
    try {
        ...
        }).populate("category", "_id category img age").populate("subCategory", "_id subCategory img age").populate("insideSubCategory", "_id insideSubCategory").populate("packType", "_id packType").populate("servingSize","_id servingSize").populate("shop","_id name address location phone email closeDate openDate status");
