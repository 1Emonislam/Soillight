        ...
        if (shop) {
            if (req?.user?.isAdmin === true) {
                ...
                if (productCreated) {
                    ...
                    const resData = await Product.findOne({ _id: productCreated?._id }).populate("category", "_id category").populate("subCategory", "_id subCategory").populate("insideSubCategory", "_id insideSubCategory").populate("packType", "_id packType").populate("servingSize","_id servingSize").populate("shop","_id name address location");
...
const productUpdate = async (req, res, next) => {
    ...
    try {
        if (!(req?.user?.role === 'seller' || req?.user?.isAdmin === true)) {
            ...
            }, { new: true }).populate("category", "_id category").populate("subCategory", "_id subCategory").populate("insideSubCategory", "_id insideSubCategory").populate("packType", "_id packType").populate("servingSize","_id servingSize").populate("shop","_id name address location");
            if (!productUpdated) {
...
const getSignleProduct = async (req, res, next) => {
    try {
        ...
        }).populate("category", "_id category").populate("subCategory", "_id subCategory").populate("insideSubCategory", "_id insideSubCategory").populate("packType", "_id packType").populate("servingSize","_id servingSize").populate("shop","_id name address location");
