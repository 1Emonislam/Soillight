...
const DashboardCounterData = async (req, res, next) => {
    try {
        ...
        const todayOrderCount = await User.find({ timestamp: { $gte: today }}).count();
        ...
        const todayOrderApprove = await User.find({ timestamp: { $gte: today }, status: 'complete' }).count();
        const todayRiderApprove = await User.find({ timestamp: { $gte: today }, role: 'rider', status: 'approved' }).count();
        const todayOrderRejected = await User.find({ timestamp: { $gte: today }, status: 'cancel' }).count();
        ...
        const lastWeekOrderCount = await User.find({ timestamp: { $gte: lastWeak } }).count();
        ...
        const lastWeekOrderApprove = await User.find({ timestamp: { $gte: lastWeak }, status: 'complete' }).count();
