...
const adminSeenOrdersSearch = async (req, res, next) => {
    if (req?.user?.isAdmin === true) {
        ...
        limit = parseInt(limit);
        try {
            const order = await Order.find({ status: status }).populate("user", "name pic").populate("products.productOwner", "name").sort({ createdAt: -1, _id: -1 }).limit(limit * 1).skip((page - 1) * limit);
