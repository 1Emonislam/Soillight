...
export default function CreateInsideSubCategory({ createInsideSubCategoryOpen, handleInsideSubCategoryOpen, handleInsideSubCategoryClose, setInsideSubCategoryOpen }) {
    ...
    useEffect(() => {
        ...
        fetch(`https://soillight-api.makereal.click/sub/category?page=1&limit=500&search=${subCategorySearch || ''}`, {
            method: 'GET',
            headers: {
                "Content-type": "application/json",
                'Authorization': `Bearer ${userLogin?.user?.token}`
    ...
    const handleClickSubCategory = (e) => {
        ...
        fetch(`https://soillight-api.makereal.click/sub/category/${e.target?.value}`, {
            method: 'GET',
            headers: {
                "Content-type": "application/json",
                'Authorization': `Bearer ${userLogin?.user?.token}`
    ...
    const createInsideSubCategory = data => {
        ...
        fetch('https://soillight-api.makereal.click/inside/sub/category/', {
            method: 'POST',
            headers: {
                "Content-type": "application/json",
                'Authorization': `Bearer ${userLogin?.user?.token}`
